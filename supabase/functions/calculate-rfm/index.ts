...
const BodySchema = z.object({ 
  ...
  job_id: z.string().uuid().optional(), // If provided, it means we are in processing mode
  is_background: z.boolean().optional() // Flag for background worker
});
...
serve(async (req) => {
  ...
  // If it's a background call from the DB, it might use service role, not user JWT
  ...
  try {
    ...
    // ─── MODE 1: ENQUEUE (Requested from Frontend) ───────────────────────
    if (!job_id && !is_background) {
      // Check for an already pending or processing job to prevent spam
      ...
      // In a real production env, we'd trigger a background worker here (via pg_net or another RPC)
      // For now, we return that it's enqueued.
      return new Response(JSON.stringify({ 
        ok: true, 
        enqueued: true, 
        job_id: job.id 
    ...
    // ─── MODE 2: PROCESSING (Background Worker) ──────────────────────────
    console.log(`Starting RFM Recalc for Job ${job_id} / Store ${store_id}`);
    ...
    const { data: customers } = await supabase.from("customers_v3").select("id").eq("store_id", store_id);
    const totalCustomers = customers?.length ?? 0;
    
    if (job_id) {
      await supabase.from("rfm_jobs").update({ total_customers: totalCustomers }).eq("id", job_id);
    }
    ...
    for (let i = 0; i < totalCustomers; i++) {
      ...
      // Update progress every 50 customers to avoid over-querying the DB
      if (job_id && i % 50 === 0) {
