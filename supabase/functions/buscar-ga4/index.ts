...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!sessionsRes.ok) { const errText = await sessionsRes.text(); throw new Error(`GA4 API error ${sessionsRes.status}: ${errText}`); }
    ...
    return new Response(JSON.stringify({ success: false, error: err instanceof Error ? err.message : "Erro interno" }), { status: 500, headers: corsHeaders });
  }
});
